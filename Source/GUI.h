...
namespace Menu {
	...
	bool experimentalColorPicker(char* text, int* r, int* g, int* b, int* a);
	char* StringToChar(std::string string);
	bool CharArray(char* option, char** cArray, int& placeHolderInt, int arraySize, std::string leftBracket, std::string rightBracket);
	bool VehicleModOption(char* option, bool b00l);
	bool CharOption(char* option, char*& output, int maxChars);
	void drawText(char* text, int font, VECTOR2 pos, VECTOR2 size, VECTOR2 textWrap, RGBA color, bool center, bool rightJustify, bool outline, int dropShadow, RGBA shadowColor, bool noScale);
	//void drawText(char* text, int font, vec2 pos, vec2 size, vec2 textWrap, rgba color, bool center, bool rightJustify, bool outline, int dropShadow, rgba shadowColor, bool noScale);
	void setOpenKey();
	void City();
...
bool isValueCloseBy(int c1, int c2);

bool isValueCloseBy2(int c1, int c2);

void RGBtoHSV(float& fR, float& fG, float fB, float& fH, float& fS, float& fV);

void HSVtoRGB(float& fR, float& fG, float& fB, float& fH, float& fS, float& fV);

void drawRect(VECTOR2 pos, VECTOR2 size, RGBA color, bool noScale);
