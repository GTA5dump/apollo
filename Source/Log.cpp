...
void Log::Init(HMODULE hModule) {
	...
	if (GetModuleFileNameA(hModule, g_logFile, MAX_PATH) != 0) {
		...
		sprintf_s(chLogBuff, szTimestamp, "INIT: Apollo");
		Menu::addText("[" + std::to_string(current_tm.tm_hour) + ":" + std::to_string(current_tm.tm_min) + ":" + std::to_string(current_tm.tm_sec) + "] " + "INIT: Apollo", true);
		if (slash != -1) {
			g_logFile[slash + 1] = '\0';
			strcpy_s(g_debugLogFile, g_logFile);
			strcat_s(g_debugLogFile, "Apollo-Log.log");
			...
			OutputDebugStringA("INIT: Apollo");
		}
		else {
			useFileOutput = false;
		}
